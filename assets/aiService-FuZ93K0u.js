import{g}from"./colorSchemes-ucW8M7Qu.js";import{g as c}from"./index-DK7d__-Z.js";const m=`{
...
${t.additionalInstructions?`ADDITIONAL INSTRUCTIONS: ${t.additionalInstructions}`:""}`}function h(t){let e=`Create a presentation about: "${t.topic}"
...
Generate the JSON now.`,e}function l(t){try{return JSON.parse(t.trim())}catch{}const e=t.match(/```(?:json)?\s*([\s\S]*?)```/);if(e)try{return JSON.parse(e[1].trim())}catch{}const r=t.indexOf("{"),i=t.lastIndexOf("}");if(r!==-1&&i>r)try{return JSON.parse(t.slice(r,i+1))}catch{}throw new Error("AI 응답에서 유효한 JSON을 추출할 수 없습니다. 다시 시도해 주세요.")}async function E(t,e){const r=g(t.colorSchemeId),i=t.orientation==="portrait"?{width:794,height:1123}:{width:1123,height:794};e?.({status:"generating",progress:10,message:"AI가 프레젠테이션을 생성하고 있습니다...",totalSlides:t.slideCount});const n=u(t),o=h(t);let s;try{t.apiKey?s=await f(t.aiEngine,t.apiKey,n,o):s=await b(t.aiEngine,n,o)}catch(d){throw e?.({status:"error",progress:0,message:d.message||"AI 생성 중 오류가 발생했습니다."}),d}e?.({status:"parsing",progress:60,message:"응답을 분석하고 있습니다...",totalSlides:s.slides.length});const a=w(s.slides);e?.({status:"rendering",progress:80,message:"슬라이드를 렌더링하고 있습니다...",totalSlides:a.length});const p={title:s.title||t.topic,description:s.description,orientation:t.orientation,colorScheme:r,designTemplateId:t.designTemplateId,canvas:i,slides:a,createdAt:new Date().toISOString()};return e?.({status:"complete",progress:100,message:"프레젠테이션이 생성되었습니다!",totalSlides:a.length}),p}async function f(t,e,r,i){if(t==="openai"){const n=await fetch("https://api.openai.com/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${e}`},body:JSON.stringify({model:"gpt-4o",messages:[{role:"system",content:r},{role:"user",content:i}],response_format:{type:"json_object"},temperature:.7,max_tokens:16e3})});if(!n.ok){const s=await n.json().catch(()=>({}));throw new Error(s.error?.message||`OpenAI API error: ${n.status}`)}const o=await n.json();return l(o.choices[0].message.content)}else{const n=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":e,"anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:16e3,system:r,messages:[{role:"user",content:i}]})});if(!n.ok){const a=await n.json().catch(()=>({}));throw new Error(a.error?.message||`Claude API error: ${n.status}`)}const s=(await n.json()).content[0].text;return l(s)}}async function b(t,e,r){const i=c();if(!i)throw new Error("Supabase가 설정되지 않았습니다.");const{data:n,error:o}=await i.functions.invoke("ppt-generate",{body:{engine:t,systemPrompt:e,userPrompt:r}});if(o)throw new Error(o.message||"Edge Function 호출 실패");if(!n?.result)throw new Error("AI 응답이 비어있습니다.");const s=typeof n.result=="string"?n.result:JSON.stringify(n.result);return l(s)}function w(t){if(!Array.isArray(t)||t.length===0)throw new Error("유효한 슬라이드 데이터가 없습니다.");return t.map((e,r)=>({...e,type:e.type||"content",pageNumber:e.pageNumber||r+1}))}async function N(t){const e=c();if(!e)throw new Error("Supabase가 설정되지 않았습니다.");const{data:{user:r}}=await e.auth.getUser();if(!r)throw new Error("로그인이 필요합니다.");const{data:i,error:n}=await e.from("ppt_presentations").insert({user_id:r.id,title:t.title,description:t.description||"",orientation:t.orientation,color_scheme_id:t.colorScheme.id,data_json:t,slide_count:t.slides.length}).select("id").single();if(n)throw new Error("저장 실패: "+n.message);return i.id}async function v(){const t=c();if(!t)return[];const{data:{user:e}}=await t.auth.getUser();if(!e)return[];const{data:r,error:i}=await t.from("ppt_presentations").select("id, title, description, orientation, color_scheme_id, slide_count, created_at, updated_at").eq("user_id",e.id).order("created_at",{ascending:!1});if(i)throw new Error("로딩 실패: "+i.message);return r||[]}async function I(t){const e=c();if(!e)throw new Error("Supabase가 설정되지 않았습니다.");const{data:r,error:i}=await e.from("ppt_presentations").select("data_json").eq("id",t).single();if(i)throw new Error("로딩 실패: "+i.message);return r.data_json}async function k(t){const e=c();if(!e)throw new Error("Supabase가 설정되지 않았습니다.");const{error:r}=await e.from("ppt_presentations").delete().eq("id",t);if(r)throw new Error("삭제 실패: "+r.message)}export{I as a,k as d,E as g,v as l,N as s};
